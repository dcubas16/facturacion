...
public class GeneradorQr {
	...
	public String generarCodigoQr(String nombreArhivo, String archivoSalida) throws Exception {
		...
		try {
			...
			Element partySupplierElement = obtenerElemento(accountingSupplier.getChildren(), "Party");
			Element partyIdentificationSupplierElement = obtenerElemento(partySupplierElement.getChildren(),
					"PartyIdentification");
			Element idSupplierElement = obtenerElemento(partyIdentificationSupplierElement.getChildren(), "ID");

			String tipDocEmisor = idSupplierElement.getAttribute("schemeID").getValue();
			String rucEmisor = idSupplierElement.getText();

			// Datos Receptor
			Element accountingCustomerElement = obtenerElemento(rootNode.getChildren(), "AccountingCustomerParty");
			Element partyCustomerElement = obtenerElemento(accountingCustomerElement.getChildren(), "Party");
			Element partyIdentificationCustomerElement = obtenerElemento(partyCustomerElement.getChildren(),
					"PartyIdentification");
			Element idCustomerElement = obtenerElemento(partyIdentificationCustomerElement.getChildren(), "ID");

			String tipDocReceptor = idCustomerElement.getAttribute("schemeID").getValue();
			String rucReceptor = idCustomerElement.getText();
...
//					rutaRetorno = ParametrosGlobales.obtenerParametros().getRutaRaiz() + Constantes.rutaImagenQr
//							+ archivoSalida + Constantes.extensionJpg;
//				if (Constantes.CONSTANTE_TIPO_DOCUMENTO_BOLETA.equals(tipDocuArchivo))
	...
	public String generarCodigoQrNotaCredito(String nombreArhivo, String archivoSalida) throws Exception {
		// Declaracion de Variables
		String nombreNodo = "", tipoImpuesto = "", montoIgv = "", digest = "", rutaRetorno = "";
		// Se crea un SAXBuilder para poder parsear el archivo
		SAXBuilder builder = new SAXBuilder();
...
//				if (Constantes.CONSTANTE_TIPO_DOCUMENTO_BOLETA.equals(tipDocuArchivo))
	...
	private Element obtenerAtributo(List<Element> listaElementos, String string, String nombreElemento) {
